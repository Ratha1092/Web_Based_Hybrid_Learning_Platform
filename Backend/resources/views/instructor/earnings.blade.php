...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="{{ route('instructor.earnings.index') }}" class="nav-item active">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
<div class="main">
    <div class="topbar">
        <div>
            ...
            <h1 class="text-2xl font-bold serif">Earnings</h1>
            <p class="text-sm text-gray-400 mt-1">Track your revenue and earnings from courses</p>
        </div>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Revenue Shares</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ $earnings->where('status', 'completed')->count() }}</div>
            <div class="stat-label">Completed Payments</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ $earnings->where('status', 'pending')->count() }}</div>
    ...
    <div class="earnings-table">
        <div class="table-header">
            <h3 class="font-semibold">Revenue History</h3>
        </div>
        <table>
            <thead>
                <tr>
            ...
            <tbody>
                @forelse($earnings as $earning)
                    <tr>
                        <td>
                            <div>
                                <div class="font-medium">{{ $earning->orderItem->order->customer_name ?? 'N/A' }}</div>
                                <div class="text-sm text-gray-400">{{ $earning->created_at->format('M j, Y') }}</div>
                            </div>
                        </td>
                        <td>
                            <div class="text-sm">{{ $earning->orderItem->course->title ?? 'N/A' }}</div>
                        </td>
                        <td>
                            <div class="amount-positive">${{ number_format($earning->total_amount, 2) }}</div>
                        </td>
                        <td>
                            <div class="amount-negative">${{ number_format($earning->platform_amount, 2) }}</div>
                        </td>
                        <td>
                            <div class="amount-positive">${{ number_format($earning->instructor_amount, 2) }}</div>
                        </td>
                        <td>
                            <span class="status-badge
                                @if($earning->status === 'completed') bg-green-500/20 text-green-400
                                @elseif($earning->status === 'pending') bg-yellow-500/20 text-yellow-400
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-8">
                            <svg class="w-12 h-12 mx-auto mb-3 text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1">
                                <line x1="12" y1="1" x2="12" y2="23"/>
                                <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
                            </svg>
                            <h3 class="text-lg font-semibold mb-2">No earnings yet</h3>
                            <p class="text-gray-400">Earnings will appear here once students purchase your courses</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    @if($earnings->hasPages())
        <div class="pagination">
            ...
            @else
                <a href="{{ $earnings->previousPageUrl() }}" class="page-link">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <polyline points="15,18 9,12 15,6"/>
                    </svg>
                </a>
            ...
            @foreach ($earnings->getUrlRange(1, $earnings->lastPage()) as $page => $url)
                @if ($page == $earnings->currentPage())
                    <span class="page-link active">{{ $page }}</span>
                @else
                    <a href="{{ $earnings->nextPageUrl() }}" class="page-link">{{ $page }}</a>
                @endif
            @endforeach

            {{-- Next Page Link --}}
            @if ($earnings->hasMorePages())
                <a href="{{ $earnings->nextPageUrl() }}" class="page-link">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
